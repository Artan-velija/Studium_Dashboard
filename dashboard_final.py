...
class Modul(ABC):
    ...
    @abstractmethod
    def get_modul_typ(self) -> str: pass
...
class DatenManager:
    def __init__(self, dateiname: str = "meine_studien_daten.json"):
        ...
        self._standard_daten = [
            ...
            # (Projekt: Pentesting wurde hier entfernt)
...
class StudienController:
    ...
    def berechne_kennzahlen(self) -> Tuple[float, int, int, float, int, float]:
        ...
        heute = datetime.now()
        start_datum = datetime(heute.year, 3, 1) 
...
class DashboardGUI:
    ...
    def _baue_ui(self) -> None:
        ...
        tk.Label(box4, text="Zeit-Budget (ab März)", bg="white", font=("Arial", 10)).pack(pady=5)
